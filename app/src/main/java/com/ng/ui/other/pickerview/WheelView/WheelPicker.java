package com.ng.ui.other.pickerview.WheelView;
...
public class WheelPicker extends View implements IDebug, IWheelPicker, Runnable {
    ...
    private Rect mRectIndicatorHead, mRectIndicatorFoot, mRectIndicatorBody;
     ...
     * 滚轮滑动时的最小/最大速度
     */
    private int mMinimumVelocity = 10, mMaximumVelocity = 8000;
    ...
    public void setTag(String tag, @DrawableRes int tagRes) {
        ...
        if (tag != null && tagRes != 0) {
            tagBitmap = BitmapFactory.decodeResource(getContext().getResources(), tagRes);
            invalidate();
        }
    }
...
//            mItemTextSize = (int) WebullTextUtils.adjustTextSize(mItemTextSize, WebullTextUtils.customeradjustsize,
//                    WebullTextUtils.customeradjustsize, settingManagerService.getFontSchemeValue());
//        }
        ...
        mPaint.setTextSize(mItemMaxTextSize);
...
/*        if (fontPath != null) {
    ...
    private int mShaderBgColor = Color.parseColor("#12B4FF");
    private LinearGradient mIndicatorLineShader;
    private LinearGradient mIndicatorBgShader;
    ...
    private void computeIndicatorRect() {
        ...
        mRectIndicatorBody.set(mRectDrawn.left, indicatorHeadCenterY,
                mRectDrawn.right, indicatorFootCenterY);

        mIndicatorLineShader = new LinearGradient(
                mRectIndicatorHead.left, mRectIndicatorHead.top,
                mRectIndicatorHead.right, mRectIndicatorHead.top,
                new int[]{getAlphaColor(0.08f, mShaderBgColor),
                        getAlphaColor(0.6f, mShaderBgColor),
                        getAlphaColor(0.08f, mShaderBgColor)},
                new float[]{0, 0.5f, 1.0f},
                Shader.TileMode.CLAMP);
        mIndicatorBgShader = new LinearGradient(
                mRectIndicatorHead.left, mRectIndicatorHead.top,
                mRectIndicatorHead.right, mRectIndicatorHead.top,
                new int[]{getAlphaColor(0.00f, mShaderBgColor),
                        getAlphaColor(0.08f, mShaderBgColor),
                        getAlphaColor(0.00f, mShaderBgColor)},
                new float[]{0, 0.5f, 1.0f},
                Shader.TileMode.CLAMP);
    }

    private int getAlphaColor(float alpha, int color) {
    ...
    protected void onDraw(Canvas canvas) {
        ...
        for (int drawnDataPos = drawnDataStartPos + mSelectedItemPosition,
            ...
            Paint.FontMetrics mBeforeMetrics = mPaint.getFontMetrics();
            //设置字体大小动态适配
            float ratio = (mDrawnCenterY - Math.abs(mDrawnCenterY - mDrawnItemCenterY) -
                    mRectDrawn.top) * 1.0F / (mDrawnCenterY - mRectDrawn.top);
            if (ratio > 0) {
                float mScaleItemTextSize = mItemTextSize + (mItemMaxTextSize - mItemTextSize) * ratio;
                mPaint.setTextSize(mScaleItemTextSize);
            } else {
                mPaint.setTextSize(mItemTextSize);
            }
            Paint.FontMetrics afterMetrics = mPaint.getFontMetrics();
            //drawnCenterY += (afterMetrics.descent - mBeforeMetrics.descent) / 2;

            int midLine = (int) (drawnCenterY - Math.abs((afterMetrics.ascent + afterMetrics.descent) / 2));
            bgSize = (int) Math.abs((afterMetrics.ascent + afterMetrics.descent));

            //绘制标记物
            if (tag != null && tagRes != 0 && tag.equals(data)) {

                if (tagBitmap != null) {
                    int strLength = getTextWidth(mPaint, data);
                    tagRect = new Rect(mDrawnCenterX + strLength,
                            midLine - bgSize / 2,
                            mDrawnCenterX + strLength + bgSize,
                            midLine + bgSize / 2);
                    mSrcRect = new Rect(0, 0, tagBitmap.getWidth(), tagBitmap.getHeight());
                    canvas.drawBitmap(tagBitmap, mSrcRect, tagRect, mPaint);
                }
            }
        ...
        if (hasIndicator) {
            ...
            mPaint.setShader(mIndicatorLineShader);
            //上下两根横线
            canvas.drawRect(mRectIndicatorHead, mPaint);
            canvas.drawRect(mRectIndicatorFoot, mPaint);
            //中间背景
            mPaint.setShader(mIndicatorBgShader);
            canvas.drawRect(mRectIndicatorBody, mPaint);
